...
from Tourney import __version__ as TOURNEY_VERSION
from Tourney.cache import cache
from Tourney.constants.themes import DEFAULT_THEME
from Tourney.models import db, get_class_by_tablename
from Tourney.plugins import get_plugin_names
from Tourney.plugins.migrations import current as plugin_current
from Tourney.plugins.migrations import upgrade as plugin_upgrade
from Tourney.utils import get_app_config, get_config, set_config, string_types
from Tourney.utils.config import get_themes
from Tourney.utils.dates import unix_time
from Tourney.utils.exports.databases import is_database_mariadb
from Tourney.utils.exports.freeze import freeze_export
from Tourney.utils.migrations import (
    create_database,
    drop_database,
...
from Tourney.utils.uploads import get_uploader
...
def import_ctf(backup, erase=True, ignore_overrides=False):
    ...
    if "db" not in member_dirs:
        ...
        raise Exception(
            'Tourney couldn\'t find the "db" folder in this backup. '
    ...
    # Check if the alembic version is from Tourney 1.x
    if alembic_version in (
        ...
        set_import_error(
            "Exception: The version of Tourney that this backup is from is too old to be automatically imported."
        )
        raise Exception(
            "The version of Tourney that this backup is from is too old to be automatically imported."
        )
    ...
    if sqlite is False and alembic_version not in get_available_revisions():
        set_import_error(
            "Exception: The target migration in this backup is not available in this version of Tourney."
        )
        raise Exception(
            "The target migration in this backup is not available in this version of Tourney."
        )
            ...
            if member.startswith("db/"):
                ...
                if data:
                    ...
                    for i, entry in enumerate(saved["results"]):
                        ...
                        # databases. Specifically between MySQL and MariaDB. Because Tourney standardizes against MySQL
    ...
    set_config("ctf_version", TOURNEY_VERSION)
