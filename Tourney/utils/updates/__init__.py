...
from Tourney.models import Challenges, Teams, Users, db
from Tourney.utils import get_app_config, get_config, set_config
from Tourney.utils.config import is_setup
from Tourney.utils.crypto import sha256


def update_check(force=False):
    """
    Makes a request to tourney.invalid to check if there is a new version of Tourney available. The service is provided in return
    ...
    if update:
        try:
            ...
            check = requests.get(
                "https://versioning.tourney.invalid/check", params=params, timeout=3
            ).json()
