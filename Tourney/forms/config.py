...
from Tourney.constants.email import (
...
from Tourney.utils.csv import get_dumpable_tables
from Tourney.utils.social import BASE_TEMPLATE
...
class LocalizationForm(BaseForm):
    default_locale = SelectField(
        "Default Language",
        description="Language to use if a user does not specify a language in their account settings. By default, Tourney will auto-detect the user's preferred language.",
...
class EmailSettingsForm(BaseForm):
    ...
    mail_server = StringField(
        "Mail Server Address",
        description="Change the email server used by Tourney to send email",
    )
