...
$(() => {
  $(".preview-challenge").click(function (_event) {
    ...
    $.get(
      Tourney.config.urlRoot + "/api/v1/challenges/" + window.CHALLENGE_ID,
      function (response) {
        const challenge_data = response.data;
        challenge_data["solves"] = null;

        $.getScript(
          Tourney.config.urlRoot + challenge_data.type_data.scripts.view,
          function () {
            $.get(
              Tourney.config.urlRoot + challenge_data.type_data.templates.view,
              function (template_data) {
                ...
                challenge_data["script_root"] = Tourney.config.urlRoot;
