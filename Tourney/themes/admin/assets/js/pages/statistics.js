...
const graph_configs = {
  "#solves-graph": {
    data: () => Tourney.api.get_challenge_solve_statistics(),
    format: (response) => {
  ...
  "#keys-pie-graph": {
    data: () => Tourney.api.get_submission_property_counts({ column: "type" }),
    format: (response) => {
      const data = response.data;
  ...
  "#categories-pie-graph": {
    data: () => Tourney.api.get_challenge_property_counts({ column: "category" }),
    format: (response) => {
      const data = response.data;
  ...
  "#solve-percentages-graph": {
    layout: (annotations) => ({
      ...
      yaxis: {
        title: `Percentage of ${
          Tourney.config.userMode.charAt(0).toUpperCase() +
          Tourney.config.userMode.slice(1)
        } (%)`,
        range: [0, 100],
      },
      annotations: annotations,
    }),
    data: () => Tourney.api.get_challenge_solve_percentages(),
    format: (response) => {
      const data = response.data;
  ...
  "#score-distribution-graph": {
    ...
    data: () =>
      Tourney.fetch("/api/v1/statistics/scores/distribution").then(
        function (response) {
          return response.json();
        },
      ),
