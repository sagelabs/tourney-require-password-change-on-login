...
const api_func = {
  users: (x, y) => Tourney.api.patch_user_public({ userId: x }, y),
  teams: (x, y) => Tourney.api.patch_team_public({ teamId: x }, y),
};
...
function toggleAccount() {
  ...
  api_func[Tourney.config.userMode](id, params).then((response) => {
    if (response.success) {
      if (hidden) {
...
function toggleSelectedAccounts(selectedAccounts, action) {
  ...
  for (let accId of selectedAccounts.accounts) {
    reqs.push(api_func[Tourney.config.userMode](accId, params));
  }
