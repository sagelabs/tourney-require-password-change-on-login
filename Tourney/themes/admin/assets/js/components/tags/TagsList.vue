...
export default {
  ...
  methods: {
    loadTags: function () {
      Tourney.fetch(`/api/v1/challenges/${this.$props.challenge_id}/tags`, {
        method: "GET",
    ...
    addTag: function () {
      if (this.tagValue) {
        ...
        Tourney.api.post_tag_list({}, params).then((response) => {
          if (response.success) {
            this.tagValue = "";
            this.loadTags();
          }
    ...
    deleteTag: function (tag_id) {
      Tourney.api.delete_tag({ tagId: tag_id }).then((response) => {
        if (response.success) {
          this.loadTags();
        }
      });
