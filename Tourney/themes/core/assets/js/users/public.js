...
Alpine.data("UserGraphs", () => ({
  ...
  async init() {
    this.solves = await Tourney.pages.users.userSolves(window.USER.id);
    this.fails = await Tourney.pages.users.userFails(window.USER.id);
    this.awards = await Tourney.pages.users.userAwards(window.USER.id);
