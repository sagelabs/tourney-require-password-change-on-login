...
Alpine.data("TeamEditModal", () => ({
  ...
  async updateProfile() {
    ...
    let response = await Tourney.pages.teams.updateTeamSettings(data);
    if (response.success) {
      this.success = true;
      this.error = false;
      setTimeout(() => {
...
Alpine.data("TeamDisbandModal", () => ({
  ...
  async disbandTeam() {
    let response = await Tourney.pages.teams.disbandTeam();
...
Alpine.data("CaptainMenu", () => ({
  ...
  async inviteMembers() {
    const response = await Tourney.pages.teams.getInviteToken();

    if (response.success) {
      const code = response.data.code;
      const url = `${window.location.origin}${Tourney.config.urlRoot}/teams/invite?code=${code}`;
...
Alpine.data("TeamGraphs", () => ({
  ...
  async init() {
    this.solves = await Tourney.pages.teams.teamSolves("me");
    this.fails = await Tourney.pages.teams.teamFails("me");
    this.awards = await Tourney.pages.teams.teamAwards("me");
