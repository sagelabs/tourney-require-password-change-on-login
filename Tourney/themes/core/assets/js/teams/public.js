...
Alpine.data("TeamGraphs", () => ({
  ...
  async init() {
    this.solves = await Tourney.pages.teams.teamSolves(window.TEAM.id);
    this.fails = await Tourney.pages.teams.teamFails(window.TEAM.id);
    this.awards = await Tourney.pages.teams.teamAwards(window.TEAM.id);
