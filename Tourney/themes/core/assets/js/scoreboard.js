...
Alpine.data("ScoreboardDetail", () => ({
  ...
  async update() {
    this.data = await Tourney.pages.scoreboard.getScoreboardDetail(10, this.activeBracket);

    let optionMerge = window.scoreboardChartOptions;
    let option = getOption(Tourney.config.userMode, this.data, optionMerge);
...
Alpine.data("ScoreboardList", () => ({
  ...
  async update() {
    this.brackets = await Tourney.pages.scoreboard.getBrackets(Tourney.config.userMode);
    this.standings = await Tourney.pages.scoreboard.getScoreboard();
  },
