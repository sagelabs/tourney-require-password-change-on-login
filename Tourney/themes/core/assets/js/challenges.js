...
Alpine.data("Hint", () => ({
  ...
  async showHint(event) {
    if (event.target.open) {
      let response = await Tourney.pages.challenge.loadHint(this.id);

      // Hint has some kind of prerequisite or access prevention
      if (response.errors) {
        event.target.open = false;
        Tourney._functions.challenge.displayUnlockError(response);
        return;
      }
      ...
      if (hint.content) {
        ...
        let answer = await Tourney.pages.challenge.displayUnlock(this.id);
        if (answer) {
          let unlock = await Tourney.pages.challenge.loadUnlock(this.id);
          ...
          if (unlock.success) {
            ...
            event.target.open = false;
            Tourney._functions.challenge.displayUnlockError(unlock);
          }
        } else {
          event.target.open = false;
        }
...
Alpine.data("Challenge", () => ({
  ...
  getStyles() {
    ...
    try {
      let size = Tourney.config.themeSettings.challenge_window_size;
      switch (size) {
        case "sm":
  ...
  async showSolves() {
    this.solves = await Tourney.pages.challenge.loadSolves(this.id);
  ...
  async showSubmissions() {
    let response = await Tourney.pages.users.userSubmissions("me", this.id);
  ...
  async showSolution() {
    ...
    Tourney._functions.challenge.displaySolution = solution => {
      this.solution = solution.html;
      new Tab(this.$el).show();
    };
    await Tourney.pages.challenge.displaySolution(solution_id);
  },
  ...
  async getShareUrl() {
    ...
    const response = await Tourney.fetch("/api/v1/shares", {
      method: "POST",
  ...
  async submitChallenge() {
    this.response = await Tourney.pages.challenge.submitChallenge(
      this.id,
      this.submission,
    );
    ...
    if (this.response.data.status === "authentication_required") {
      window.location = `${Tourney.config.urlRoot}/login?next=${Tourney.config.urlRoot}${window.location.pathname}${window.location.hash}`;
      return;
    }
  ...
  async renderSubmissionResponse() {
    ...
    if (this.getSolutionId() == null) {
      if (
        Tourney.pages.challenge.checkSolution(
          this.getSolutionState(),
          Alpine.store("challenge").data,
          this.response.data.status,
        )
      ) {
        let data = await Tourney.pages.challenge.getSolution(this.id);
  ...
  async submitRating() {
    const response = await Tourney.pages.challenge.submitRating(
      this.id,
      this.selectedRating,
      this.ratingReview,
    );
...
Alpine.data("ChallengeBoard", () => ({
  ...
  async init() {
    this.challenges = await Tourney.pages.challenges.getChallenges();
    this.loaded = true;
  ...
  getCategories() {
    ...
    try {
      const f = Tourney.config.themeSettings.challenge_category_order;
      if (f) {
  ...
  getChallenges(category) {
    ...
    try {
      const f = Tourney.config.themeSettings.challenge_order;
      if (f) {
        const getSort = new Function(`return (${f})`);
  ...
  async loadChallenge(challengeId) {
    await Tourney.pages.challenge.displayChallenge(challengeId, challenge => {
