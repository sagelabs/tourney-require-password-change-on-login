...
const displayChal = chal => {
  return Promise.all([
    Tourney.api.get_challenge({ challengeId: chal.id }),
    $.getScript(config.urlRoot + chal.script),
    $.get(config.urlRoot + chal.template)
  ]).then(responses => {
    const challenge = Tourney._internal.challenge;
...
function renderSubmissionResponse(response) {
  ...
  const next_btn = $(
    ...
    setTimeout(function() {
      loadChal(Tourney._internal.challenge.data.next_id);
    }, 500);
  });
  ...
  if (result.status === "authentication_required") {
    ...
    if (Tourney._internal.challenge.data.next_id) {
...
function getSolves(id) {
  return Tourney.api.get_challenge_solves({ challengeId: id }).then(response => {
...
function loadChals() {
  return Tourney.api.get_challenge_list().then(function(response) {
    const categories = [];
...
const displayUnlock = id => {
  ezQuery({
    ...
    success: () => {
      ...
      Tourney.api.post_unlock_list({}, params).then(response => {
        if (response.success) {
          Tourney.api.get_hint({ hintId: id }).then(response => {
