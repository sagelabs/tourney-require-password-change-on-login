# Dynamic Value Challenges for Tourney

It's becoming commonplace in CTF to see challenges whose point values decrease
after each solve.

This Tourney plugin creates a dynamic challenge type which implements this
...
Within Tourney you are free to mix and match regular and dynamic challenges.
...
![](https://raw.githubusercontent.com/Tourney/DynamicValueChallenge/master/function.png)
...
**REQUIRES: Tourney >= v1.2.0**

1. Clone this repository to `Tourney/plugins`. It is important that the folder is
   named `DynamicValueChallenge` so Tourney can serve the files in the `assets`
   directory.
