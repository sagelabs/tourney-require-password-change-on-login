...
from Tourney.constants.config import (
...
from Tourney.utils import config, get_config, set_config
from Tourney.utils import user as current_user
from Tourney.utils import validators
from Tourney.utils.config import can_send_mail, is_setup, is_teams_mode
from Tourney.utils.config.pages import build_markdown, get_page
from Tourney.utils.config.visibility import challenges_visible
from Tourney.utils.dates import ctf_ended, ctftime, view_after_ctf
from Tourney.utils.decorators import authed_only
from Tourney.utils.health import check_config, check_database
from Tourney.utils.helpers import get_errors, get_infos, markup
from Tourney.utils.modes import USERS_MODE
...
from Tourney.utils.uploads import get_uploader, upload_file
from Tourney.utils.user import authed, get_current_team, get_current_user, get_ip, is_admin
...
def setup():
    ...
    if not config.is_setup():
        ...
        if request.method == "POST":
            ...
            if ctf_banner:
                ...
                banner_location = url_for("views.files", path=f.location)
                header_html = (
                    '<img class="w-100 mx-auto d-block" '
                    'style="max-width: 500px;padding: 50px;padding-top: 14vh;" '
                    f'src="{banner_location}" />'
                    f'<h1 class="text-center">{escape(ctf_name)}</h1>'
                )
            else:
                header_html = (
                    '<h1 class="text-center" style="padding-top: 20vh;">'
                    f"{escape(ctf_name)}</h1>"
                )
    ...
    <div class="col-md-6 offset-md-3">
        ...
        <h4 class="text-center">
            <a href="challenges">Challenges</a> &middot; <a href="scoreboard">Scoreboard</a>
        </h4>
    </div>
</div>"""
            page.content = index
...
def themes_beta(theme, path):
    ...
    In Tourney 4.0 this url_for behavior and this themes_beta
