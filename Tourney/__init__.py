...
import Tourney.utils.config
from Tourney import utils
from Tourney.constants.themes import ADMIN_THEME, DEFAULT_THEME
from Tourney.plugins import init_plugins
from Tourney.utils.crypto import sha256
from Tourney.utils.initialization import (
    init_cli,
    init_events,
    init_logs,
...
from Tourney.utils.migrations import create_database, migrations, stamp_latest_revision
from Tourney.utils.sessions import CachingSessionInterface
from Tourney.utils.updates import update_check
...
class TourneyFlask(Flask):
    def __init__(self, *args, **kwargs):
        ...
        self.request_class = TourneyRequest
    ...
    def create_jinja_environment(self):
        ...
        return super(TourneyFlask, self).create_jinja_environment()
    ...
    def create_url_adapter(self, request):
        ...
        return super(TourneyFlask, self).create_url_adapter(request)
...
def confirm_upgrade():
    if sys.stdin.isatty():
        print("/*\\ Tourney has updated and must update the database! /*\\")
        print("/*\\ Please backup your database before proceeding! /*\\")
        print("/*\\ Tourney maintainers are not responsible for any data loss! /*\\")
...
def create_app(config="Tourney.config.Config"):
    app = TourneyFlask(__name__)
    with app.app_context():
        app.config.from_object(config)

        from Tourney.cache import cache
        from Tourney.utils import import_in_progress
        ...
        while import_in_progress():
            print("Import currently in progress, Tourney startup paused for 5 seconds")
            time.sleep(5)
        ...
        for theme_name in Tourney.utils.config.get_themes():
        ...
        from Tourney.models import (  # noqa: F401
            Challenges,
            Fails,
            Files,
            Flags,
        ...
        if reverse_proxy:
            if type(reverse_proxy) is str and "," in reverse_proxy:
                ...
                # TODO: Tourney 4.0 We should deprecate this behavior and require that users specify the level of control they want
        ...
        # Upgrading from an older version of Tourney
        ...
        from Tourney.api import api
        from Tourney.auth import auth
        from Tourney.challenges import challenges
        from Tourney.errors import render_error
        from Tourney.events import events
        from Tourney.scoreboard import scoreboard
        from Tourney.share import social
        from Tourney.teams import teams
        from Tourney.users import users
        from Tourney.views import views
