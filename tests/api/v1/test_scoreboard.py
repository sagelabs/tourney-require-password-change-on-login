...
from Tourney.cache import clear_standings
...
def test_scoreboard_is_cached():
    ...
    with app.app_context():
        ...
        saved = app.cache.get("Tourney.utils.scoreboard.get_scoreboard_detail_memver")
        ...
        with login_as_user(app, "user1") as client:
            ...
            assert (
                app.cache.get("Tourney.utils.scoreboard.get_scoreboard_detail_memver")
                == saved
            )
            cached = client.get("/api/v1/scoreboard/top/10").get_json()
            assert cached["data"] == orig
            assert app.cache.get("Tourney.utils.scoreboard.get_scoreboard_detail_memver")
            ...
            new = app.cache.get("Tourney.utils.scoreboard.get_scoreboard_detail_memver")
            assert new != saved
    destroy_tourney(app)
