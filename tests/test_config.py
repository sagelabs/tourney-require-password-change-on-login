...
from Tourney.models import Configs, Users, db
...
def test_preset_admin_config():
    ...
    app = create_tourney(config=PresetAdminConfig)
    with app.app_context():
...
def test_preset_admin_token_config():
    ...
    app = create_tourney(config=PresetAdminTokenConfig)
    with app.app_context():
        # Verify no preset admin exists initially
...
def test_preset_admin_empty_credentials():
    ...
    app = create_tourney(config=PresetAdminEmptyPasswordConfig)
    with app.app_context():
        # Verify no preset admin exists initially
    ...
    app = create_tourney(config=PresetAdminEmptyTokenConfig)
    with app.app_context():
        # Verify no preset admin exists initially
...
def test_preset_configs():
    ...
    app = create_tourney(config=PresetConfigsConfig)
    with app.app_context():
