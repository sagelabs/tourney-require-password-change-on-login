...
from tests.helpers import create_tourney, destroy_tourney, gen_user, login_as_user
...
def test_that_tourney_can_be_deployed_in_subdir():
    """Test that Tourney can be deployed in a subdirectory"""
    ...
    app = create_tourney(setup=False, application_root="/ctf")
    with app.app_context():
        with app.test_client() as client:
            r = client.get("/")
...
def test_that_tourney_subdir_redirects_work():
    ...
    app = create_tourney(config=ApplicationRootConfig, application_root="/ctf")
    with app.app_context():
...
def test_that_request_path_hijacking_works_properly():
    """Test that the TourneyRequest subclass correctly mimics the Flask Request when it should"""
    app = create_tourney(setup=False, application_root="/ctf")
    assert app.request_class.__name__ == "TourneyRequest"
    with app.app_context():
...
def test_theme_fallback_config():
    ...
    app = create_tourney(config=ThemeFallbackConfig)
    # Make an empty theme
    try:
